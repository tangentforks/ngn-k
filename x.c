...
Z A2(fA,/*01*/N n=xn;y=ff(n,y);F(n|!n,y=f(xa,y))y)
Z A2(f,/*01*/y=apc(y,xt);
 X(
   RmMA(fA(x,y))
   RE(y=ff(xL[0],y);ff(xL[1],y))
   RS(y=ff(xn,y);F(xn,y=f(as(xi),y))y)
...
Z A0(gA,L n=h8;P(!(n<(W)(1ll<<45)),ed0())A x=aA(n);F(n|!n,P(!(xa=g()),mrn(i,xA);AZ(x);x(0)))x)
Z A0(g,C t=h1;P(!(t<(W)tn),ed0())
 S(t,
  ...
  RS(L n=h8;P(!(n<(W)(1ll<<45)),ed0())A x=aS(n);F(n,xi=Nx(g()))x)
  Rs(N n=h1;sym(h(n)))
  R6(ti,tc,tu,tv,tw,tx,h8)
  R4(to,tp,tq,tr,C c=h1;A x=N(gA());AT(t,AK(c&7,t==tr?AW(c>>3,x):x)))
  R_(L n=h8;P(!(n<(W)(1ll<<45)),ed0())aV(t,n,h(n*Tw[t]))))0)
