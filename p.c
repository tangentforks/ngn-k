...
Z S s0,s,sr;Z U k;Z A pb(A,C);                                                                         //parser state (s:current pointer, s0:start of source, sr:start of last statement, k:implicit arg counter)
...
Z A pt(C*v)_(C c=*s;                                                                                //parse term
 ...
 P(c>>7,S p=s;A x=pP();*v=1;AO(p-sr,x))
...
Z X1(pm,                                                                                            //monadify
 ...
 R_(x))
A pT(C*v)_(A x=N(pt(v));                                                                          //parse term and the adverbs or square brackets after it (v:verb?)
 W(1,C c=*s;U i=si("'/\\[",c);P(i>3,x)s++;
  I(i>2,x=AO(s-1-sr,N(pb(x,']')));I(xn==2,I(xy==GAP,xy=au)E(xx=pm(xx)))*v=0)
...
Z A pe(A x,C*v)_(s=pw(s);C c=*s;                                                                    //parse expression
 ...
 UH o=s-sr;C b=0;A y=pT(&b);P(!y,x?x(0):0)P(y==GAP,x?x:y)
