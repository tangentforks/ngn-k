...
A1(prsC,XB(N n=xn;A u=MINE(x)?xR:aV(tB,n,xV);uT=tC;un=n+7>>3;I(n%8u,uC[un-1]&=-1+(1<<(n%8u)));F(un+3>>2,Fj(4,uL[4*i+j]=bswp(uL[4*i+j])))x(u))et0())
Z A1(cgB,XB(P(xn>8,el0())UC b=xC[0];b<<=-xn%8u;b=(b&0xF0)>>4|(b&0x0F)<<4;b=(b&0xCC)>>2|(b&0x33)<<2;b=(b&0xAA)>>1|(b&0x55)<<1;x(ac(b)))et0())
A1(bytes,XA(e1f(cgB,x))et0())
