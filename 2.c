...
Z A addZZ(A x,A y,U f)_(XB(addZZ(cG(xR),cG(y),f))U w=MAX(xw-3,yw-3);P(xw-3-w,x=ct(tG+w,xR);x(addZZ(x,y,f)))y=ct(tG+w,y);U n=yn;A z=an(n,yt);
...
Z A addzZ(L v,A y,U f)_(P(!v&&ytZ,y=sqzZ(y);yT=tG+yw-3;y)U n=yn,w=MAX(tZ(v)-tG,yw-3);y=ct(tG+w,y);A z=an(n,yt);G(&agG,ahH,aiI,alL)[w](v,yV,zV,n+(31>>w)>>5-w);
