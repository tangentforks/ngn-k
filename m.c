...
A aa_(U n,A x,B c/*n?c*/)_(P(MINE(x)&&((W)n<<xw)+7>>3<=cap(x),AN(n,c?x:xR))A y=an(n,xt);MC(yV,xV,((W)xn<<Tw[xt])+7>>3);I(ytR,I(MINE(x),AZ(x))E(mRn(xn,xA)))c?x(y):y)//realloc
...
A mut_(A x,B c)_(XP(x)P(MINE(x),x)A y=aV(xt,xn,xV);x=c?x(y):y;XR(mRa(x))x)
A1(mut,mut_(x,1))
