...
Z L maxfZ(L v,A x/*0*/)_(MAX(v,G(&maxfB,0,0,maxfG,maxfH,maxfI,maxfL)[xw](xV,xn)))
Z A3(admf,/*011*/B i=xv==3;U n=zn;P((y&&ytf)||ztF,F v=y?gf(cF(y)):i;z=cF(z);Mz(I(i,F(n,v*=zf))E(F(n,v+=zf)))af(v))L v=y?gl(y):i;z(az((i?mulfZ:addfZ)(v,z))))
...
Z A3(___s,/*011*/U i=!y;A u;I(i,y=ii(z,0);u=enl(yR))E(yR;u=emp(tG))U n=zn;W(i<n,y=x2(y,ii(z,i++));P(!y,z(u(0)))PSH(u,yR))z(y(u)))
Z A3(dexs,/*011*/I(y,y(0))z)
Z A3(adms,/*011*/L w=y?gl(y):x==MUL;U n=zn;I b=1;L v=w;C t=tG+zw-3;A u=an(n,t);
 I(x==ADD,F4(zw-3,n,ug=v+=zg;B(v-(G)v,b=0),uh=v+=zh;B(v-(H)v,b=0),ui=v+=zi;B(v-(I)v,b=0),ul=v+=zl))
 E(       F4(zw-3,n,ug=v*=zg;B(v-(G)v,b=0),uh=v*=zh;B(v-(H)v,b=0),ui=v*=zi;B(v-(I)v,b=0),ul=v*=zl))P(b,z(u))z=ct(t+1,u(z));adms(x,az(w),z))
Z A3(subs,/*011*/y=neg(y?y:mul(ai(2),ii(z,0)));neg(adms(ADD,y,z)))
Z A3(mxms,/*011*/P((!y||ytz)&&ztZ,L v=y?gl(y):-WL,l=-1ll<<(1<<zw)-1,h=~l;U n=zn;I(v<=l||h<=v,P(v>=0,z(rsz(n,az(v))))v=v<0?l:h)
                                  A u=an(n,zt);F4(zw-3,n,ug=v=MAX(v,zg),uh=v=MAX(v,zh),ui=v=MAX(v,zi),ul=v=MAX(v,zl))z(u))___s(x,y,z))
Z A3(mnms,/*011*/P((!y||ytz)&&ztZ,z=inv(z);inv(mxms(MXM,y?az(~gl(y)):0,z)))___s(x,y,z))
/* Z A eqlsL(L v,A x/\*0*\/)_(A y=aG(xn);F4(xw-3,xn,yg=v=v==xg,yg=v=v==xh,yg=v=v==xi,yg=v=v==xl)y) */
Z A3(eqls,/*011*/U n=zn,i=!y;L v=gl(y?y:io(z,0)),a=v;A u=aG(n);S4(zw-3,W(i<n,ug=v=v==zg;i++),W(i<n,ug=v=v==zh;i++),W(i<n,ug=v=v==zi;i++),W(i<n,ug=v=v==zl;i++))z(0);y||!n?u:a4(u,ai(0),av,az(a)))
A3(ars,/*011*/Q(xtv)Q(xv<11)Q(!y||ytzc)Q(ztZC)
 ZE(z=gZ(z);ars(x,y,z))
 ZB(z=cG(z);ars(x,y,z))
...
Z A3(cmpp,/*011*/I o=x-LTN,w=zw-3;U n=zn;A u=aG(n);L v=gl(y),p=iw(z,w,0);*uG=!o?p<v:o==1?p>v:p==v;L m=n-1,j=m;
 S4(o,F4(w,m,uG[j]=zG[j]< zG[j-1];j--,uG[j]=zH[j]< zH[j-1];j--,uG[j]=zI[j]< zI[j-1];j--,uG[j]=zL[j]< zL[j-1];j--),
      ...
      F4(w,m,uG[j]=zG[j]==zG[j-1];j--,uG[j]=zH[j]==zH[j-1];j--,uG[j]=zI[j]==zI[j-1];j--,uG[j]=zL[j]==zL[j-1];j--),)z(u))
A3(arp,/*011*/Q(xtv)Q(xv<11)Q(ytzc)Q(ztZC)
 ZE(z=gZ(z);arp(x,y,z))
 ZB(z=cG(z);arp(x,y,z))
 G(&dexp,___p,___p,___p,___p,modp,mnmp,mxmp,cmpp,cmpp,cmpp)[xv](x,y,z))

Z C tZx(A x)_(C t=TX[xt];t?t:tZ(gl_(x)))
C sup_(A*p,A*q,B c)_(A x=*p,y=*q;C t=MAX(tZx(x),tZx(y));*p=x=Ny(ct(t,c?x:xR));*q=y=Nx(ct(t,y));t)
