...
Z A frS(I f,N n)_(C b[1024];I m=fm(f);B r=S_ISREG(m);A x=emp(tC);I k=1;W(n&&k,k=f?read(f,b,MIN(SZ b,n)):ipc_stdin_read(b,MIN(SZ b,n));P(k<0,eo(x))n-=k;x=cts(x,b,k);P((!r&&-1==n+k||f<3)&&k-SZ b,x))x) // read stream (only length)
